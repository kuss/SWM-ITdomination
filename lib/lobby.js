...
Lobby.prototype.addLobbyListener = function (user) {
	...
	user.on("lobbyChat", function(content){
		lobby.broadcast(user.place, "lobbyLog", user.name + " : "+ content );
	});
}
...
Lobby.prototype.movePlace = function (user, place) {
	if (user.place !== -1){
		this.broadcast(user.place, "lobbyLog", user.name + " 님께서 이동 하셨습니다.");
	...
	this.broadcast(user.place, "lobbyLog", user.name + " 님께서 참가 하셨습니다.");
