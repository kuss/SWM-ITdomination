...
Lobby.prototype.destroyGame = function(game) {
	for (var i in game.players) {
		...
		this.movePlace(game.players[i], Place[0]);
...
var GameHandlers = [
	{
		event : "click"
		,handler : function(playerId, cardId) {
	...
	,{
		...
		,handler : function() {
			return game.turnEnd(player);
		}
	}
	,{
		event : "chat"
		,handler : function() {
			return game.chat(player.proto.name + " : "+ content);
		}
	}
];
Lobby.prototype.addGameListener = function (player, game) {
	for (var i in GameHandlers) {
		player.proto.on(GameHandlers[i].event, GameHandlers[i].handler);	
	}
}
...
Lobby.prototype.movePlace = function (user, place) {
	if (user.place !== -1){
		...
		Util.deletebyName(Place[user.place].users, user);
	}
