	...
	return function(card, game, player){
		for (var i in player.front_field) {
			if (player.front_field[i].proto.name === name || name === undefined || name === null) {
				...
				if (player.front_field[i].atk < 0) player.front_field[i].atk = 0;
			}
		}
		return true;
	}
		...
		for (var i in game.players) {
			if (player !== game.players[i]) {
				for (var j in game.players[i].front_field) {
					...
					if (game.players[i].front_field[j].atk < 0) game.players[i].front_field[j].atk = 0;
				}
			}
		}
	}
...
exports.removeCardfromBackField = function (target) {
	return function(card, game, player) {
		for (var card in player.back_field) {
			if (target === undefined || target === null) {
				game.removeCardfromField(player, null);
			}
			else if (player.back_field[card].proto.name === target) {
				game.removeCardfromField(player.back_field, player.back_field[card]);
			}
		}
	}
}
exports.destroyCardfromHand = function (target) {
	return function(card, game, player) {
		for (var card in player.hand) {
			...
			else if (card.proto.name === target) {
...
exports.getCardfromDeck = function (targetcard) {
	return function(card, game, player) {
		for (var i in player.deck) {
			if (player.deck[i].proto.name === targetcard || targetcard === undefined || targetcard === null)
