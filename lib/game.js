...
Game.prototype.getCardfromDeck = function (player, card, other) {
	if ((card === undefined || card === null) && (other === undefined || other === null)) {
		...
		Util.shiftbyName(cardQueue[RandomIndex].own.deck, cardQueue[RandomIndex]);
		...
		Util.shiftbyName(other.deck, card);
...
Game.prototype.getCardfromTomb = function (player, card, other) {
	if ((card === undefined || card === null) && (other === undefined || other === null)) {
		...
		Util.shiftbyName(cardQueue[RandomIndex].own.tomb, cardQueue[RandomIndex]);
		Util.push(player.hand, cardQueue[RandomIndex]);
		getcard.own = player.id;
		return true;
	}
		...
		Util.shiftbyName(other.tomb, card);
		Util.push(player.hand, card);
		card.own = player.id;
		return true;
	}
...
Game.prototype.removeCardfromDeck = function (player, card) {
	if (player === null) {
		for (var player in players) {
			Util.shiftbyName(players[player].deck, card);
			Util.push(player.tomb, card);
			return true;
		}
	}
	else {
		Util.shiftbyName(player.deck, card);
		Util.push(player.tomb, card);
		return true;
	}
