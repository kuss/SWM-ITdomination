...
Game.prototype.getCardfromDeck = function (player, card, other) {
	if ((card === undefined || card === null) && (other === undefined || other === null)) {
		...
		Util.shiftbyProto(cardQueue[RandomIndex].own.deck, cardQueue[RandomIndex]);
...
Game.prototype.getCardfromTomb = function (player, card, other) {
	if ((card === undefined || card === null) && (other === undefined || other === null)) {
		...
		Util.shiftbyProto(cardQueue[RandomIndex].own.tomb, cardQueue[RandomIndex]);
		Util.push(player.hand, cardQueue[RandomIndex]);
		getcard.own = player.id;
		return true;
	}
...
Game.prototype.getCardfromHand = function (player, card, other) {
	if ((card === undefined || card === null) && (other === undefined || other === null)) {
		...
		Util.deletebyProto(cardQueue[RandomIndex].own.hand, cardQueue[RandomIndex]);
		Util.push(player.hand, cardQueue[RandomIndex]);
		getcard.own = player.id;
		return true;
	}
...
Game.prototype.removeCardfromDeck = function (player, card) {
	if (player === null) {
		for (var player in players) {
			Util.shiftbyProto(players[player].deck, card);
			Util.push(player.tomb, card);
			return true;
		}
	}
	else {
		Util.shiftbyProto(player.deck, card);
		Util.push(player.tomb, card);
		return true;
	}
...
Game.prototype.applyEffects = function (state) {
	// TODO
	// 업데이트 되어야 하는 것
	// 플레이어의 턴당 머니
	// 기본 시제품 카드 공격력/수비력
...
/*	this.market.vit = this.market.proto.vit;
	...
	for (var player_i in this.players) {
		for (var card in players[player_i].hand) {
			for (var i in players[player_i].hand[card].proto.effects) {
				// TODO : Card turn제 확인
				if (players[player_i] === player) {
					if (players[player_i].hand[card].proto.effect[i].condition(state)) {
						// 현재 효과를 발동한 사람의 상태
					}
				}
				if (players[player_i].hand[card].proto.effects[i].condition(Constant.state.notTurn)) {
					// 효과를 발동하기 전 사람의 상태
				}
			}
		}
