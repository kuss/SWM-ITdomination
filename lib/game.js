...
Game.prototype.screenfy = function (player) {
	...
	console.log("스크린에 들어간 콘솔");
	console.log(screen.hand);
	screen.enemy = {
		front_field : []
		,back_field : []
	}; 
	for(var i in enemy.front_field){
		screen.enemy.front_field.push({
			card : enemy.front_field[i]
			,action : []
		});
	}
	for(var i in enemy.back_field){
...
Game.prototype.sendScreen = function(player) { //player에게 현재 스크린을 업데이트한다. player가 없으면 전체 업데이트 
	if(player == undefined || player == null){
		for (var i in this.players){
			this.players[i].send("setScreen", this.screenfy(this.players[i]));
		}
	}
	else{
...
Game.prototype.gameStart = function () {
	...
	for (var i in this.players){
		...
		this.players[i].send("message", "덱에서 카드를 "+Constant.game.initHand+"장 가져옵니다.");
	}
...
Game.prototype.turnStart = function (player) { //턴 시작! 
	...
	player.send("message", "덱에서 카드를 한장 가져옵니다.");
...
Game.prototype.putProductCard = function (player, card) {
	...
	if (Util.length(player.front_field) >= Constant.game.maxFrontFieldCard) {
		...
		if (this.useMoney(player, card.cost)) {
			...
			player.send("message", "시제품 &lt;"+card.proto.name+"&gt;을(를) 개발했습니다.");
...
Game.prototype.putSkillCard = function (player, card) {
	...
	if (skillCard >= Constant.game.maxSkillCard) {
		...
		if (this.useMoney(player, card.cost)) {
			console.log("BEEEEEFFFOROROOREOEEO");
			ddd(player.hand);
			Util.shiftbyName(player.hand, card);
			console.log("AFFFFFFFFFFFFFFFFFTER");
			ddd(player.hand);

			Util.push(player.back_field, card);
			player.send("message", "기술 카드 &lt;"+card.proto.name+"&gt;을(를) 사용하였습니다.");
			return true;
		}
		else {
			Util.raiseError("Not enough money");
...
Game.prototype.putTacticCard = function (player, card) {
	...
	if (tacticCard >= Constant.game.maxTacticCard) {
		...
		if (this.useMoney(player, card.cost)) {
			Util.shiftbyName(player.hand, card);
			Util.push(player.back_field, card);
			player.send("message", "전략 카드 &lt;"+card.proto.name+"&gt;을(를) 사용하였습니다.");
			return true;
		}
		else {
			Util.raiseError("Not enough money");
...
Game.prototype.attack = function (player, card) {
	...
	if (this.market === null) {
		if (this.useMoney(player, player.attackCost)){
			...
			player.send("message", "당신의 &lt;" +card.proto.name+ "&gt;이(가) 시장 장악에 성공했습니다!");
		this.market = card;
			return true;
		}
		else {
	...
	if (this.useMoney(player, player.attackCost)) {
		...
		player.send("message", "시장에 있는 적 &lt;"+this.market.proto.name+"&gt;에게 "+card.atk+"의 데미지를 입혔습니다.");
...
Game.prototype.increaseOcc = function (player) {
	...
	if (player.id === this.market.own) {
		...
		player.send("message", "당신의 &lt;"+this.market.proto.name+"&gt;이(가) 시장을 "+this.market.occ+"%만큼 추가로 점유했습니다.");
...
Game.prototype.increaseMoney = function (player) {
	...
	player.send("message", "\\ "+player.moneyPerTurn + "만큼의 자금을 벌었습니다.");
}
