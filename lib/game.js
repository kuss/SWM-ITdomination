...
Game.prototype.attack = function (player, card) {
	...
	if (useMoney(player, card.cost)) {
  	...
  	if (this.market.vit <= 0) {
			...
			Util.deletebyProto(player.front_field, card);
			this.market = card;
			return true;
		}
		else {
			Util.deletebyProto(player.field, card);
			return false;
		}
	}
	else {
...
Game.prototype.applyEffects = function () {
	...
	for (var player in this.players) {
		...
		for (var card in players[player].hand) {
			players[player].hand[card].cost = players[player].hand[card].proto.cost;
		}
	}
	// market카드 기본 설정
/*	this.market.vit = this.market.proto.vit;
	...
	for (var player in this.players) {
		for (var card in players[player].hand) {
			for (var i in players[player].hand[card].proto.effects) {
				// TODO : Card turn제 확인
				if (players[player].hand[card].proto.effects[i].condition()) {
